# Universal Mcp Asana MCP Server

An MCP Server for the Universal Mcp Asana API.
...
```bash
mcp dev src/universal_mcp_asana/mcp.py
```
...
This is automatically generated from OpenAPI schema for the Universal Mcp Asana API.


Here is the markdown table summarizing the tools in the `AsanaApp` class:

| Tool | Description |
|------|-------------|
| `get_an_allocation` | Retrieves a complete allocation record for a single allocation. |
| `update_an_allocation` | Updates an existing allocation by making a PUT request. |
| `delete_an_allocation` | Deletes a specific, existing allocation and returns an empty data record upon success. |
...
| `upload_an_attachment` | Uploads an attachment to an object and returns the created attachment's compact record. |
...
| `update_acustom_field` | Updates an existing custom field with provided data, returning the complete updated record. |
| `delete_acustom_field` | Deletes a specific existing custom field using a DELETE request to its URL. |
...
| `reorder_acustom_field_senum` | Reorders a custom field's enum. |
| `update_an_enum_option` | Updates an existing enum option for custom fields and returns the full record. |
...
| `update_agoal` | Update an existing goal by sending a PUT request to the specified URL. |
...
| `get_multiple_portfolios` | Retrieve multiple portfolios, returning them in a compact representation. |
...
| `get_multiple_portfolio_memberships` | Retrieves multiple portfolio memberships, returning them in a compact representation. |
...
| `get_multiple_projects` | Retrieve multiple projects based on filtering criteria, returning compact project records. |
...
| `get_task_count_of_aproject` | Retrieves the task count of a project, returning an object with task count fields. |
...
| `add_followers_to_aproject` | Adds followers to a project, promoting specified users to members if not already part of the project. |
...
| `delete_aproject_brief` | Deletes a specific existing project brief (deprecated: new integrations should use '/project_briefs/{project_brief_gid}'). |
...
| `get_aproject_status` | Get the complete record for a single project status update. |
...
| `delete_asection` | Deletes a specific empty section by sending a DELETE request to its URL. |
...
| `add_task_to_section` | Adds a task to a specific section in a project, removing it from other sections. |
...
| `get_astatus_update` | Get a status update by requesting a complete record from an endpoint. |
...
| `get_astory` | Retrieves a story's full record, returning all data fields except those excluded by default. |
| `update_astory` | Updates a story and returns the full record of the updated story. |
...
| `get_multiple_tags` | Retrieve multiple tags with optional pagination and filtering. |
...
| `update_atag` | Updates a tag's properties by sending a PUT request with specified optional fields. |
...
| `update_atask` | Updates a specific, existing task by making a PUT request. |
| `delete_atask` | Deletes a specific task permanently after moving it to the user's trash (recoverable for 30 days). |
...
| `get_tasks_from_asection` | Retrieves tasks from a specified section, primarily designed for board views. |
| `get_tasks_from_atag` | Retrieve paginated tasks associated with a specific tag. |
...
| `add_aproject_to_atask` | Associates a task with a project, optionally positioning it relative to other tasks or within a specific section. |
...
| `get_multiple_task_templates` | Retrieve multiple task templates with pagination and filtering options. |
...
| `add_auser_to_ateam` | Adds a user to a team and returns the complete team membership record. |
| `remove_auser_from_ateam` | Remove a user from a team, requiring team membership in the calling user. |
| `get_ateam_membership` | Get a team membership record. |
...
| `get_auser` | Get a user record by ID, returning the full user details. |
...
| `get_users_in_ateam` | Retrieve paginated list of team members, returning compact user records sorted alphabetically with a limit of 2000 results. |
| `get_users_in_aworkspace_or_organization` | Retrieve paginated list of users in a workspace or organization, sorted alphabetically and limited to 2000 records. |
| `get_auser_task_list` | Retrieves the full record for a user task list. |
...
| `delete_awebhook` | Permanently deletes a webhook. |
| `get_multiple_workspaces` | Fetches multiple workspaces visible to the authorized user. |
...
| `remove_auser_from_aworkspace_or_organization` | Remove a user from a workspace or organization. |
...
- Visit Apps Store and enable the Universal Mcp Asana app
